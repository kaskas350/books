                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            <div class="product-item-detail-info-section">
                                ...
                                <? if (CSite::InGroup([1, 6])): //если пользователь админиcтратор или зарегистрировавшийся?>
                                    <? if (!empty($arResult["ID_USER"]) )://если книга у кого-то или только что текущий пользователь взял её себе?>
                                       <? if ($arResult["ID_USER"] == CUser::GetId())://если книга у текущего пользователя?>
                                            <div>Книга у вас в наличии</div>
                                        <? else :// если книга не у текущего пользователя?>
                                            <? COption::SetOptionString("main", "component_cache_on", "N"); ?>

                                            <? if (CSite::InGroup([1]))://если текущий пользователь администратор?>
                                                <div>
                                                    Книга находится у пользователя
                                                </div>
                                                <div>
                                                    Логин: <?= $arResult["US"]["LOGIN"] ?>
                                                </div>
                                                <? if ($arResult["US"]["NAME"]): ?>
                                                    <div>
                                                        Имя: <?= $arResult["US"]["NAME"] ?>
                                                    </div>
                                                    <div>
                                                        Фамилия: <?= $arResult["US"]["LAST_NAME"] ?>
                                                    </div>
                                                <? endif ?>
                                            <? else: ?>
                                            ...
                                            <? if (($arResult["ID_TURN"] !== "")): ?>
                                                ...
                                                <div> Книга уже находится в очереди
                                                    <? if (($arResult["ID_TURN"] == CUser::GetId())): ?>
                                                        у вас
                                                    <? else : ?>
                                                        <? if (CSite::InGroup(array(1)))://если текущий пользователь администратор?>
                                                            у пользователя  <?= $arResult["US_TURN"]["NAME"] . "  " . $arResult["US_TURN"]["LAST_NAME"] ?>
                                                        <? else : ?>
                                                        <? endif ?>
                                                    <? endif ?>
                                                </div>
                                            <? else : ?>
                                                <div><a href="?turn=Y&CODE=<?= $arResult["ID_BOOK"] ?>">Записаться в
                                                        очередь на книгу</a></div>
                                            <? endif ?>
                                        <? endif ?>
                                    <? else: ?>
                                        <a data-toggle="modal" data-target="#productModal"
                                           title="Взять книгу из библиотеки" class="quick-view modal-view detail-link"
                                           href="?rec=Y&CODE=<?= $arResult["ID_BOOK"] ?>">Взять книгу</a>
                                    <?print_r($arResult);?>
                                    <? endif ?>
                                <? endif ?>
                            </div>
