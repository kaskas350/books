				...
				<div class="row">
					<div class="col-sm-6">
						<div class="product-item-detail-info-section">
                            ...
                            $id_book=$arResult["ORIGINAL_PARAMETERS"]["ELEMENT_ID"]; //ид текущей книги
                            $m= CIBlockElement::GetProperty($arResult["ORIGINAL_PARAMETERS"]["IBLOCK_ID"],$id_book);
                            $id_user=$arResult["PROPERTIES"]["IN_STOCK"]["VALUE"];  //ид пользователя у которого книга
                            $id_turn=$arResult["PROPERTIES"]["IN_TURN"]["VALUE"];  //ид пользователя на очередь к книги
                            $us=(CUser::GetById($id_user))->NavNext(); // пользователь у которого книга
                            $us_turn=(CUser::GetById($id_turn))->NavNext(); // пользователь в очередь на книгу

                           foreach($arResult["PROPERTIES"] as $k=>$m):
                               if (($k=="IN_STOCK")||($k=="IN_TURN")||($k=="DATE_OF_RECEIPT")||($k=="RETURN_DATE"))
                                {
                                   continue;
                                }
                                ?>
                            <div style="width: 400px"> <?=$m['NAME']?>:<?=$m['VALUE']?> </div><br>
                            <?endforeach;?>


    <?if (CSite::InGroup(array(1,6))): //если пользователь админимтратор или зарегистрировавшийся?>

          <?if (    ($id_user!=="")    ||  $y    )://если книга у кого-то или только что текущий пользователь взял её себе?>



                <?if (($id_user==CUser::GetId()) || $y)://если книга у текущего пользователя?>

                    <div>Книга у вас в наличии</div>

                <?else :// если книга не у текущего пользователя?>

                <?
                COption::SetOptionString("main", "component_cache_on", "N");?>


                            <?if (CSite::InGroup(array(1)) )://если текущий пользователь администратор?>
                              <div>
                                  Книга находится у пользователя
                              </div>
                               <div>
                                  Логин: <?=$us["LOGIN"]?>
                               </div>
                ...
                <div>
                    Фамилия: <?=$us["LAST_NAME"] ?>

                </div>
                      <?endif?>
                             <?else: ?>

                    <div>Книги нет в наличии</div>
                             <?endif?>


                <?if (($id_turn!=="") || $t):?>

                    <div> Книга уже находится в очереди
                        <?if (($id_turn==CUser::GetId()) || $t):?>
                            у вас
                        <?else :?>
                            <?if (CSite::InGroup(array(1)) )://если текущий пользователь администратор?>
                                у пользователя  <?=$us_turn["NAME"]."  ".$us_turn["LAST_NAME"]?>
                            <?else :?>
                            <?endif?>
                        <?endif?>
                    </div>
                ...
                <?else :?>
                    ...
                    <div><a href="?turn=Y&CODE=<?=$id_book?>">Записаться в очередь на книгу</a></div>
          ...
          <?else: ?>
                ...
                <a data-toggle="modal" data-target="#productModal" title="Взять книгу из библиотеки" class="quick-view modal-view detail-link" href="?rec=Y&CODE=<?=$id_book?>">Взять книгу</a>
          <?endif?>
