...
if (CModule::IncludeModule("iblock")) {
    if ($arParams["IBLOCK_ID"] > 0) {
        $bWorkflowIncluded = CIBlock::GetArrayByID($arParams["IBLOCK_ID"],
                "WORKFLOW") == "Y" && CModule::IncludeModule("workflow");
    } else {
    ...
    if ($USER->IsAdmin() || is_array($arGroups) && is_array($arParams["GROUPS"]) && count(array_intersect($arGroups,
            $arParams["GROUPS"])) > 0) {
        $bAllowAccess = true;
    ...
    if ($bAllowAccess) {
        ...
        if ($USER->GetID()) {
            ...
            $rsIBLockPropertyList = CIBlockProperty::GetList(array("sort" => "asc", "name" => "asc"),
            ...
            if ($arParams["ELEMENT_ASSOC"] == "PROPERTY_ID" && intval($arParams["ELEMENT_ASSOC_PROPERTY"]) > 0 && in_array($arParams["ELEMENT_ASSOC_PROPERTY"],
                    $arPropertyIDs)) {
            ...
            if (check_bitrix_sessid() && $_REQUEST["delete"] == "Y" && $arResult["CAN_DELETE"]) {
                ...
                // удалять только в том случае, если удаляет текущий владелец этой кникги
                $m = GetIBlockElement($_REQUEST["CODE"]);

                if (empty($m["PROPERTIES"]["IN_TURN"]["VALUE"])) {
                    CIBlockElement::SetPropertyValues($_REQUEST["CODE"], ID_BLOCK, [
                        "IN_STOCK" => "",
                        "RETURN_DATE" => FormatDateFromDB()
                    ]);
                } else {
                    CIBlockElement::SetPropertyValues($_REQUEST["CODE"], ID_BLOCK, [
                        "IN_STOCK" => $m["PROPERTIES"]["IN_TURN"]["VALUE"],
                        "DATE_OF_RECEIPT" => FormatDateFromDB(),
                        "IN_TURN" => ""
                    ]);
                }
            }
            ...
            if ($bWorkflowIncluded) {
                ...
                $arResult["ACTIVE_STATUS"] = array(
                    "Y" => GetMessage("IBLOCK_FORM_STATUS_ACTIVE"),
                    "N" => GetMessage("IBLOCK_FORM_STATUS_INACTIVE")
                );
            }
            ...
            if ($arParams["NAV_ON_PAGE"] < $arResult["ELEMENTS_COUNT"]) {
                $arResult["NAV_STRING"] = $rsIBlockElements->GetPageNavString(GetMessage("IBLOCK_LIST_PAGES_TITLE"), "",
                    true);
            }
            ...
            while ($arElement = $rsIBlockElements->NavNext(false)) {
                ...
                if ($bWorkflowIncluded) {
                    ...
                    $arElement["CAN_EDIT"] = $arResult["CAN_EDIT"] == "Y" ? (in_array($arElement["WF_STATUS_ID"],
                        $arParams["STATUS"]) == true ? "Y" : "N") : "N";
                    $arElement["CAN_DELETE"] = $arResult["CAN_DELETE"] == "Y" ? (in_array($arElement["WF_STATUS_ID"],
                        $arParams["STATUS"]) == true ? "Y" : "N") : "N";
