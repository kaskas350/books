	...
	if ($bAllowAccess)
	{
		...
		if ($USER->GetID())
		{
			...
			if (check_bitrix_sessid() && $_REQUEST["delete"] == "Y" && $arResult["CAN_DELETE"]) {
                ...
                if ($m["PROPERTIES"]["IN_TURN"]["VALUE"]=="")
                {
                    CIBlockElement::SetPropertyValueCode($_REQUEST["CODE"], "IN_STOCK", "");
                    CIBlockElement::SetPropertyValueCode($_REQUEST["CODE"], "RETURN_DATE", FormatDateFromDB());

                } else {

                    CIBlockElement::SetPropertyValueCode($_REQUEST["CODE"],"IN_STOCK", $m["PROPERTIES"]["IN_TURN"]["VALUE"]);
                    CIBlockElement::SetPropertyValueCode($_REQUEST["CODE"], "DATE_OF_RECEIPT", FormatDateFromDB());
                    CIBlockElement::SetPropertyValueCode($_REQUEST["CODE"],"IN_TURN", "");
